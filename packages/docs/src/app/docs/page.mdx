...
<Tabs items={['Demo', 'Code']}>
  ...
  <Tabs.Tab>
    ```jsx copy
    <Application className='hover:cursor-grab active:cursor-grabbing'>
      <EnvAtlasComponent src='/environment-map.png' intensity={0.5}/>
      <Grid />
      <Entity name='camera' position={[4, 2, 4]}>
          ...
          <OrbitControls 
            inertiaFactor={0.07} 
            distanceMin={6} distanceMax={10} 
            pitchAngleMin={1} pitchAngleMax={90}
          />
          <StaticPostEffects />
          <AutoRotate />
      </Entity>
...
The library is currently beng use in product at SnapChat for 

<Cards>
  <Cards.Card
    icon={<Icons.snapchat />}
    title="SnapChat"
    href="https://ml.snapchat.com"
    arrow
  />
</Cards>
