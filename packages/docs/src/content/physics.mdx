export const NUM_SHAPES = 50;
export const SHAPES = [...Array(NUM_SHAPES)].map(() => ({ scale: [0.75, 0.75, 1, 1, 1.25][Math.floor(Math.random() * 5)] * 2 }))
export const randRange = range => range * (0.5 - Math.random())
...
export const Physics = () => {
    ...
    const app = useApp();
    const { data: diffuseMap } = useTexture('/materials/paint/diffuse.webp');


    const matA = useMaterial({ diffuse: "#c0a0a0",emissive: "red" });
    const matB = useMaterial({ diffuseMap: diffuseMap?.resource, gloss: 0.98});
    ...
    return (<Entity>
        ...
        { SHAPES.map(({ scale }, i) => (<ShapeCollider 
            key={i} 
            position={[randRange(40), randRange(40) - 50, randRange(40) - 20]}
            scale={scale} 
            material={materials[i % materials.length]}
        >
            <Script script={MoverScript} scale={scale * 0.3}/>
            <RigidBody type="dynamic" linearDamping={0.75} angularDamping={0.15} friction={0.2}/>
        </ShapeCollider>)) }
    </Entity>)
}
