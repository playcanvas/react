...
const renderWithProviders = (ui: ReactNode) => {
    return render(<Application deviceTypes={['null']}>{ui}</Application>);
};
...
describe('Entity', () => {
    ...
    it.skip('validates prop types correctly', async () => {
        ...
        renderWithProviders(<Entity position={[1, 2, 3, 4] as unknown as [number, number, number]} />);

        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Invalid value for prop "position"'));

        renderWithProviders(<Entity rotation={[1, 2, 3, 4] as unknown as [number, number, number]} />);
