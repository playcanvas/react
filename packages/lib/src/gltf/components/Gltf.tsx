...
import type { Rule, MergedRule, ModifyComponentAction } from '../types.ts';
...
export const Gltf: React.FC<GltfProps> = ({ asset, render = true, children }) => {
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line react-hooks/set-state-in-effect -- the cloned hierarchy becomes effect-owned state
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line react-hooks/set-state-in-effect -- rule processing synchronizes the cached hierarchy
        processRules();
 ...
 */
function buildHierarchyCache(entity: Entity, parentPath: string, cache: Map<string, EntityMetadata>): void {
