...
import type { PathPredicate, EntityMetadata, PathMatcher } from '../utils/path-matcher.ts';
 ...
 * ```
 */
export function useEntity(path: string | PathPredicate): Entity | Entity[] | null {
    ...
    return useMemo(() => {
        ...
        const hasWildcards =
            typeof path === 'function' || (typeof path === 'string' && (path.includes('*') || path.includes('[')));
 ...
 */
function findEntitiesByPattern(rootEntity: Entity, pattern: string, pathMatcher: PathMatcher): Entity[] {
    const matches: Entity[] = [];
