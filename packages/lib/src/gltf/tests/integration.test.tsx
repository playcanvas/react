...
import type { Entity as PcEntity, Asset, LightComponent, RenderComponent } from 'playcanvas';
import { Application as PcApplication, NullGraphicsDevice } from 'playcanvas';
...
describe('Gltf Integration Tests', () => {
    ...
    describe('Adding children', () => {
        ...
        it('should add multiple children', async () => {
            ...
            // --- ACT ---
            render(
                <AddMultipleChildren asset={asset} path="RootNode.Body" childNames={['Child1', 'Child2', 'Child3']} />
            );
        ...
        it('should mix functional and direct prop updates', async () => {
            ...
            render(
                <Application deviceTypes={['null']}>
                    <Gltf asset={asset} key={asset.id}>
                        <Modify.Node path="RootNode.Body.Head">
                            <Modify.Light intensity={(val: number | undefined) => (val || 1) * 2} color="blue" />
                        </Modify.Node>
                    </Gltf>
                </Application>
            );
            ...
            render(
                <Application deviceTypes={['null']}>
                    <Gltf asset={asset} key={asset.id}>
                        <Modify.Node path="RootNode.Body.Head">
                            <Modify.Light
                                intensity={(val: number | undefined) => {
    ...
    describe('Multiple rules and conflict resolution', () => {
        it('should apply multiple non-conflicting rules', async () => {
            ...
            // --- ACT ---
            render(
                <MultipleNonConflictingRules asset={asset} path1="RootNode.Body.Head" path2="RootNode.Body.LeftArm" />
            );
        ...
        describe('Warnings', () => {
            it('should warn when adding component to entity that already has that component', async () => {
                ...
                try {
                    ...
                    const lightWarning = warnSpy.mock.calls.find(
                        (call) =>
                    ...
                    const renderWarning = warnSpy.mock.calls.find(
                        (call) =>
                            call[0] === '%c[PlayCanvas React]:' &&
                            typeof call[1] === 'string' &&
                            typeof call[2] === 'string' &&
