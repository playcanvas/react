...
import { findEntityByName, countEntities, getAllPaths } from '../../../test/utils/gltf-entity-builder.ts';
...
describe('Hierarchy Cache Building', () => {
    ...
    describe('Edge cases', () => {
        ...
        it('should handle multiple entities with same name', () => {
            ...
            const headsInCache = Array.from(cache.values()).filter((m) => m.name === 'Head');
    ...
    describe('Cache querying', () => {
        it('should allow finding entities by component type', () => {
            ...
            const entitiesWithLight = Array.from(cache.values()).filter((m) => m.entity.c?.light);
