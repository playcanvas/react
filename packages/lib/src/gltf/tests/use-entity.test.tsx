...
import { createSimpleRobot, createComplexScene, createLightingScene } from '../../../test/fixtures/gltf-hierarchies.ts';
...
import type { EntityMetadata } from '../utils/path-matcher.ts';
import { PathMatcher } from '../utils/path-matcher.ts';

/**
 * Helper function to build hierarchy cache (extracted from Gltf component)
 */
function buildHierarchyCache(entity: PcEntity, parentPath: string, cache: Map<string, EntityMetadata>): void {
 ...
 */
function GltfWrapper({ rootEntity, children }: { rootEntity: PcEntity; children: React.ReactNode }) {
    ...
    return (
        <GltfContext.Provider value={contextValue}>
            <ParentContext.Provider value={rootEntity}>{children}</ParentContext.Provider>
    ...
    describe('Predicate functions', () => {
        it('should find entities matching predicate', async () => {
            ...
            render(
                <Application deviceTypes={['null']}>
                    <GltfWrapper rootEntity={hierarchy}>
                        <TestComponent path={(entity) => entity.c?.light !== undefined} onResult={onResult} />
                    </GltfWrapper>
                </Application>
            );
        ...
        it('should find entities by name predicate', async () => {
            ...
            render(
                <Application deviceTypes={['null']}>
                    <GltfWrapper rootEntity={hierarchy}>
                        <TestComponent path={(entity) => entity.name === 'Head'} onResult={onResult} />
                    </GltfWrapper>
                </Application>
            );
        ...
        it('should return null when predicate matches nothing', async () => {
            ...
            render(
                <Application deviceTypes={['null']}>
                    <GltfWrapper rootEntity={hierarchy}>
                        <TestComponent path={(entity) => entity.name === 'NonExistent'} onResult={onResult} />
                    </GltfWrapper>
                </Application>
            );
