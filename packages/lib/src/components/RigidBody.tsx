...
import { BODYTYPE_STATIC, BODYTYPE_DYNAMIC, BODYTYPE_KINEMATIC, Entity } from 'playcanvas';
...
export const RigidBody: FC<RigidBodyProps> = (props) => {
    ...
    const safeProps = validatePropsPartial(
        props,
        componentDefinition as ComponentDefinition<RigidBodyProps, RigidBodyComponent>
    );
    ...
    useComponent(
        isPhysicsLoaded ? 'rigidbody' : null,
        safeProps,
        componentDefinition.schema as Schema<RigidBodyProps, RigidBodyComponent>
    );
...
componentDefinition.schema = {
    ...
    type: {
        ...
        errorMsg: (value: unknown) =>
            `Invalid value for prop "type": ${value}. Expected one of: "${rigidBodyTypes.join(', ')}".`,
        default: 'static'
    }
};
