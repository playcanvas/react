 ...
 * ```
 */
export function useAppEvent<TEventMap = BaseEventCallbackMap, TEventName extends keyof TEventMap = keyof TEventMap>(
    event: TEventName,
...
export const useFrame = (callback: (dt: number) => void) => {
    warnOnce(
        "`useFrame` is deprecated and will be removed in a future release. Please use useAppEvent('update', callback) instead."
    );
