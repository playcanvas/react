...
const createAppWithUpdateCollector = (canvas: HTMLCanvasElement) => {
    ...
    const app = {
        on: (evt: string, cb: () => void) => {
            if (evt === 'update') updateHandlers.push(cb);
        },
        off: vi.fn(),
        scene: {},
...
describe('usePicker', () => {
    ...
    it('Entity registers/unregisters pointer events and picker responds only when present', async () => {
        ...
        vi.spyOn(app.root, 'findComponents').mockReturnValue([
            { priority: 1, renderTarget: null }
        ] as unknown as pc.CameraComponent[]);
        ...
        const Harness = ({ withEntity }: { withEntity: boolean }) => {
            const elRef = useRef<HTMLDivElement | null>(null);
            useEffect(() => {
                elRef.current?.appendChild(appCanvas);
            }, []);
